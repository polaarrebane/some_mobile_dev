...
import com.ronia.fr.module00.ex03.calculator.Calculator
...
fun CalculatorKeyboard() {
    val calculator = CalculatorProvider.current
    Column {
        Row(modifier = Modifier.fillMaxWidth().height(48.dp))
        {
            CalcButton("7", { calculator.onDigit(7) }, ButtonType.Numeric)
            CalcButton("8", { calculator.onDigit(8) }, ButtonType.Numeric)
            CalcButton("9", { calculator.onDigit(9) }, ButtonType.Numeric)
            CalcButton("C", { calculator.onC() }, ButtonType.Reset)
            CalcButton("AC", { calculator.onAC() }, ButtonType.Reset)
        }

        Row(modifier = Modifier.fillMaxWidth().height(48.dp))
        {
            CalcButton("4", { calculator.onDigit(4) }, ButtonType.Numeric)
            CalcButton("5", { calculator.onDigit(5) }, ButtonType.Numeric)
            CalcButton("6", { calculator.onDigit(6) }, ButtonType.Numeric)
            CalcButton("+", { calculator.onAction("+") }, ButtonType.Action)
            CalcButton("−", { calculator.onAction("-") }, ButtonType.Action)
        }

        Row(modifier = Modifier.fillMaxWidth().height(48.dp))
        {
            CalcButton("1", { calculator.onDigit(1) }, ButtonType.Numeric)
            CalcButton("2", { calculator.onDigit(2) }, ButtonType.Numeric)
            CalcButton("3", { calculator.onDigit(3) }, ButtonType.Numeric)
            CalcButton("×", { calculator.onAction("*") }, ButtonType.Action)
            CalcButton("/", { calculator.onAction("/") }, ButtonType.Action)
        }
        ...
        Row(modifier = Modifier.fillMaxWidth().height(48.dp))
        {
            CalcButton("0", { calculator.onDigit(0) }, ButtonType.Numeric)
            CalcButton("•", { calculator.onDot() }, ButtonType.Numeric)
            CalcButton("00", { calculator.onDigit(0); calculator.onDigit(0) }, ButtonType.Numeric)
            CalcButton("=", { calculator.onEqual() }, ButtonType.Equal)
        }
    }
}
...
fun ScrollContent(innerPadding: PaddingValues) {
    ...
    Column(
        ...
        DisplayText(calculator.currentInput, calculator.result)
        CalculatorKeyboard()
    }
}
...
val CalculatorProvider = compositionLocalOf<Calculator> {
    error("There is no Calculator")
}
...
fun App() {
    ...
    CompositionLocalProvider(CalculatorProvider provides calculator) {
        MaterialTheme(
